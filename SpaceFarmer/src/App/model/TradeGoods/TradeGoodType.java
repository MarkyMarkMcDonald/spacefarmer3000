...
public enum TradeGoodType {
	...
	public int determineQuantity(Planet planet) {
		...
		if (lowCondition.ordinal()==resource.ordinal()) {
			quantity= (int) (REDUCED_QUANTITY*(Randomizer.nextInt(
					AVERAGE_QUANTITY-MINIMUM_QUANTITY)+MINIMUM_QUANTITY));
		}
		else if (highCondition.ordinal()==resource.ordinal()) {
			quantity= (int) (INCREASED_QUANTITY*(Randomizer.nextInt(
					AVERAGE_QUANTITY-MINIMUM_QUANTITY)+MINIMUM_QUANTITY));
		}
		else {
			quantity = (int) (Randomizer.nextInt(AVERAGE_QUANTITY-MINIMUM_QUANTITY) + 
					MINIMUM_QUANTITY);
		}
